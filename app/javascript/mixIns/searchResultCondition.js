  ...
  computed: {
   ...
   filterItems() {
     ...
     switch (this.filterType) {
        case Config.HEADER_NAME.name:
          if (this.search) {
            tmpList = this.items.filter(item => item.testFormHeaders.header_name.indexOf(this.search) != - 1);
          } else {
            tmpList = this.items
          }
          break;
        case Config.TEST_TYPE.name:
          if (this.testType.length){
            tmpList = this.items.filter(item => this.testType.indexOf(item.testFormHeaders.test_type) >= 0);
          } else {
            tmpList = this.items
          }
          break;
        case Config.USER_ID.name:
          if (this.userId.length){
            tmpList = this.items.filter(item => this.userId.indexOf(item.testFormHeaders.user_id) >= 0);
          } else {
            tmpList = this.items
          }
          break;
        case Config.CREATED_AT.name:
          if (this.createdTimeType == Config.SAME_DAY.name)   {
            tmpList = this.items.filter(item => this.isSame(this.$moment(item.testFormHeaders.created_at).format("YYYY/MM/DD"), this.getCreateAt));
          } else if(this.createdTimeType == Config.BEFORE_DAY.name) {
            tmpList = this.items.filter(item => this.isBefore(this.$moment(item.testFormHeaders.created_at).format("YYYY/MM/DD"), this.getCreateAt));
          } else {
            tmpList = this.items.filter(item => this.isAfter(this.$moment(item.testFormHeaders.created_at).format("YYYY/MM/DD"), this.getCreateAt));
          }
          break;
        case Config.ID.name:
          if (this.testFormId){
            tmpList = this.items.filter(item => this.testFormId ==  item.testFormHeaders.test_form_header_id);
          } else {
            tmpList = this.items
          }
          break;
